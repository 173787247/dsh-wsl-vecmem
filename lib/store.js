...
export function createStore(storePath) {
  ...
  return {
    ...
    clear(workspace) {
      ...
      if (!ws) {
        const before = load().items.length;
        save({ version: 1, items: [] });
        return { removed: before, remaining: 0 };
      }
      const db = load();
      const before = db.items.length;
      db.items = db.items.filter((it) => it.meta?.workspace !== ws);
      save(db);
      return { removed: before - db.items.length, remaining: db.items.length };
    },
    all() {
      return load().items;
    },
  ...
  return {
    ...
    async search({ query, topK = 5, workspace } = {}) {
      ...
      return {
        ok: true,
        workspace: ws || null,
        hits: scored.slice(0, Math.min(20, Math.max(1, topK))),
      };
    },
    clear({ workspace } = {}) {
      const ws = normalizeWorkspace(workspace);
      ...
      return {
        ok: true,
        workspace: ws || null,
        cleared: ws ? "workspace" : "all",
        removed: result.removed,
        remaining: result.remaining ?? store.list().length,
      };
    },
  };
}
